...
<div id="user-edit">
  <Modal v-model="modal.visible" :title="modal.title" footer-hide>
    <IForm :key="modal.visible" :model="model" :elem="userElem" :rules="userRule" :loading="loading.form" :btn-loading="loading.btn" :width="360" :label-width="80" @on-submit="handleSubmit" @on-click="modal.visible = false" button button-text="Cancel" />
    <!-- IForm -->
  </Modal>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      payload.birth = payload.birth ? this.$Utils.formatDate.format(new Date(payload.birth), 'yyyy-MM-dd') : ''
      // 模拟异步请求(编辑 Or 新增)
      setTimeout(() => {
