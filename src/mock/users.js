...
export default () => {
  ...
  Mock.mock(/\/user-list/, config => {
    const {pagePara, name} = qs.parse(config.url.split("?")[1]);
    ...
    current = current > pageMax ? pageMax : current;
  ...
  Mock.mock(/\/user-edit/, config => {
    const {id, name, age, gender, email, birth, city, hobby, desc} = qs.parse(
      config.body
    );
  ...
  Mock.mock(/\/user-create/, config => {
    const {name, age, gender, email, birth, city, hobby, desc} = qs.parse(
      config.body
    );
    const _params = params(city, hobby);
    users.unshift({
